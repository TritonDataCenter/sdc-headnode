...
#
# ruby_app_update_service() will create, import and enable the update service
# for the given application. It requires the application name as its single
# argument.
#
function ruby_app_update_service
{
  local name=$1

  echo "Creating $name-update service"

  cat >"/opt/local/share/smf/manifest/$name-update.xml" <<SMF
<?xml version='1.0'?>
<!DOCTYPE service_bundle SYSTEM '/usr/share/lib/xml/dtd/service_bundle.dtd.1'>
<service_bundle type='manifest' name='export'>
  <service name='platform/smartdc/${name}-update' type='service' version='0'>
    <create_default_instance enabled='true'/>
    <single_instance/>
    <dependency name='filesystem' grouping='require_all' restart_on='error' type='service'>
      <service_fmri value='svc:/system/filesystem/local'/>
    </dependency>
    <exec_method name='start' type='method' exec='/opt/smartdc/$name-data/$name-update-service.sh' timeout_seconds='60'>
      <method_context>
    ...
    <template>
      <common_name>
        <loctext xml:lang='C'>SmartDC Update service ($name)</loctext>
      </common_name>
    </template>
  </service>
</service_bundle>
